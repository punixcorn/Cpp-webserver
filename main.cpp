...
template <typename T>
concept str = requires(T a) {
    (std::is_same_v<decltype(a), const char *> ||
     std::is_same_v<decltype(a), std::string>) == true;
...
class webserver {
   ...
   public:
    webserver(T Host, int Port) : host(Host), port(Port){};
   ...
   private:
    ...
    void post_request(int client_socket, int content_lenght,
        ...
        while (std::getline(stream, line, '&')) {
            ...
            if (pos != std::string::npos) {
                parsed_data.insert({line.substr(0, pos), line.substr(pos + 1)});
            }
        }
    ...
    int get_server_fd() {
        ...
        if (server_socket == -1) {
            std::print(stderr, "Failed to create server socket\n");
            return -1;
        }
        ...
        if (bind(server_socket, (struct sockaddr *)&server_address,
            ...
            std::print(stderr, "Failed to bind socket\n");
            close(server_socket);
            return -1;
        }
    ...
    void server_listen(int server_socket) {
        if (listen(server_socket, 1) == -1) {
            std::print(stderr, "server Failed to listen\n");
            close(server_socket);
        }
    ...
    void runServer() {
        ...
        try {
            for (;;) {
                ...
                if (client_socket == -1) {
                    std::print(stderr, "Err falied to accept connection\n");
                    continue;
                }
                ...
                if (bytes == 0 || bytes == -1) {
                    std::print(stderr, "Error recieving data\n");
                    close(client_socket);
                    continue;
                }
                ...
                // === === === //
                std::print("request_data : {} \n", request_data);
                // === === === //
                auto [method, path] = parse_request(request_data);

                std::print("recieved {} from {}\n", method, path);
                ...
                if (method == "GET")
                    ...
                    std::print("method not implemented yet\n");

            }  // for
        } catch (const std::exception &e) {
            std::print(stderr, "{}\n", e.what());
        } catch (...) {
            std::print(stderr, "ERR: Something happened, but we dunno\n");
        }
        close(server_socket);
    }
};
...
int main(int argc, char **argv) {
    webserver Server("127.0.0.1", 8000);
